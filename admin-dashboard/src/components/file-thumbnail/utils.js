...
export function fileData(file) {
  ...
  if (typeof file === 'object' && 'hash' in file) {
    return {
      key: file?.formats?.thumbnail?.hash ? file?.formats?.thumbnail?.hash : file?.hash,
      preview: file?.formats?.thumbnail?.url ? file?.formats?.thumbnail?.url : file?.url,
      name: file.formats.thumbnail.name,
      type: fileTypeByUrl(file.formats.thumbnail.url),
    };
  }
