...
def publish(id, league_name, featured_image, title, text_all, types, round, language, league_id):  # list_id[global_for]
    ...
    if platform_name == 'wordpress':
        ...
        if category != "" \
                and data_j['category_id'] != "":
            category = [category, data_j['category_id']]
        elif category == "" \
                and data_j['category_id'] != "":
            category = data_j['category_id']
...
def main_publication(rounds, types, league_id):
    ...
    for global_for in range(len(list_id)):
        ...
        if types == 'review':
            ...
            max_total_passes_with_accurate_in_round = variables_json["team_efficiency"][
            ...
            min_total_passes_with_accurate_in_round = variables_json["team_efficiency"][
            ...
            name_teams_cards_top3_fouls_of_season = variables_json["players_stats"][
            ...
            h3_fixture_match_list = variables_json["tops"]["h3_fixture_match_list"].replace("[", "").replace("]",
                                                                                                             "").replace(
                "'", "").replace(",", "")
            h3_amounts_list = variables_json["tops"]["h3_amounts_list"].replace("[", "").replace("]", "").replace("'",
                                                                                                                  "").replace(
                ",", "").split()
            ...
            if client_language == 'eng':
                ...
                for i in range(len(h3_team_names_list)):
                    ...
                    if i == 0:
                        ...
                        h3_leader_opneAI = h3_leader_opneAI + f'{name_players_for_h3.replace("_", " ")} ({team_names.replace("_", " ")}) scored {h3_amounts_list[i]} in the match with {team_away.replace("_", " ")}. His team {result} with the final score {goals_home_h3} - {goals_away_h3}. '
                    if i != 0:
                        h3_another = h3_another + f'{name_players_for_h3.replace("_", " ")} ({team_names.replace("_", " ")}) scored {h3_amounts_list[i]}, '
                        # h3_another_openAI = h3_another_openAI + f'{name_players_for_h3.replace("_", " ")} ({team_names.replace("_", " ")}) scored {h3_amounts_list[i]}, '
                h3_another = h3_another[:-2]

                # h3_another_openAI = h3_another_openAI[:-2]
                """ TABLE_league """
                tr_1 = (
            ...
            """
            openai.api_key = "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"
            """ Preview """
            list_text = [
                f"Write a headline using the example.\nExample: Match day {rounds} in {league_name} ended today.",
                f"Write a text about this data in the example. \nExample: The last match of the Match Day {rounds} in {league_name} has ended. The leader — {team_home_leader} played with {team_away_rival} and won {result_game_leader} {goal_leader}-{goal_rival}. Home teams have {count_home} wins and away teams won {count_away} times. {count_draw} matches ended in a draw.",
                f"Write a text about this data in the example. \nExample: {h3_leader_opneAI} All the teams scored {all_goals_round} goals ({all_penalty_round} from penalties). It is {total_percent_round}% to the previous Match Day. In general, teams score {average_goals_in_season} goals per Match Day. \n\nThe fastest goal of this Match Day was scored at {time_fast_goal} minute by {name_fast_goal} ({team_name_fast_goal}) during the game with {team_away_fast_goal}. The final score of the match {scrore_fast_goal}.",
                f"Write a text about this data in the example. \nExample: Destroyer of the Match Day — {team_top_destroyer}. The players did {destroyer_interceptions} interceptions, {destroyer_blocks} blocks, {destroyer_tackles} tackles, and the goalkeeper made {destroyer_saves} saves. In total — {amount_max_destroyer} destroying actions. {team_main_destroyer} {destroyer_results} — {team_rival_destroyer}. The final score is {goals_main_destroyer} - {goals_rival_destroyer}. The destroyer of the season — {max_destroyer_of_season_name} ({max_destroyer_of_season_amount} activities per Match Day).",
                f"Write a text about this data in the example. \nExample: Creator of the Match Day — {team_top_creator}. The players won {creator_duels} duels, made {creator_shots_on_target} shots on target and {creator_shots_off_target} shots off target. In total — {amount_max_creator} actions. This team played with {team_rival_creator} ({goals_main_creator} - {goals_rival_creator}). The creator of the season — {max_creator_of_season_name} ({max_creator_of_season_amount} this kind of actions per Match Day).",
                f"Write a text about this data in the example. \nExample: Best pass accuracy — {name_max_accurate_in_round} ({max_accurate_in_round}% of {max_total_passes_with_accurate_in_round} passes). The best team in the league — {name_max_accuracy_in_season} ({percent_max_accuracy_in_season}% of the passes per Match Day). The worst team with pass accuracy — {name_min_accurate_in_round} ({min_accurate_in_round}% of the passes). The worst team this season — {name_min_accuracy_in_season} ({percent_min_accuracy_in_season}% pass accuracy per Match Day).",
                f"Write a text about this data in the example. \nExample: {name_max_saves_of_round} ({main_team_max_saves}) made more saves during the Match Day — {round_max_saves_of_round} ({date_game_max_saves}). Hi did it during the game with {rival_team_max_saves}. His team won {goals_and_info_max_saves}. As to fouls — the match with the most cards shown ({top_fouls_total_yel_card} yellow and {top_fouls_total_red_card} red) is the game between {top_fouls_team_name_home} and {top_fouls_team_name_away} (final score — {top_fouls_goals_home}-{top_fouls_goals_away}).",
            ]
            ...
            for text in list_text:
                ...
                temperature_amount = 1 if index_openai == 1 else 0.7
                ...
                # print(type(response))
                if index_openai != 1:
                    main_text_list.append(response["choices"][0]["text"])
                else:
                    title_openAI = response["choices"][0]["text"]
                # print(response["choices"][0]["text"])

            for i in ['"', "'"]:
                title_openAI = title_openAI.replace(i, "") if i in title_openAI else title_openAI
            first_paragraph, second_paragraph, threeth_paragraph, fourth_paragraph, fiveth_paragraph, sixth = main_text_list

            l1 = ['{first_paragraph}', '{second_paragraph}', '{threeth_paragraph}', '{fourth_paragraph}', '{fiveth_paragraph}', '{sixth}',
                  '{rounds}', '{league_name}', '{all_rounds}', '{team_home_leader}', '{team_away_rival}',
                  ...
                  '{name_teams_cards_top3_fouls_of_season[2]}', '{destroyer_results}', '{date_game_max_saves}',
                  '{city_next_round}']
            l2 = [first_paragraph, second_paragraph, threeth_paragraph, fourth_paragraph, fiveth_paragraph, sixth,
                  rounds, league_name, all_rounds, team_home_leader, team_away_rival, goal_leader, goal_rival,
            ...
            Делаем публикацию
            _____"""
            result_publish = publish(list_id[global_for], league_name, featured_image, title, text_all, types, rounds,
                                     client_language, league_id)
            try:
                if 'telegram' in data_j['auto-posting'] and result_publish != False:
                    ...
                    for i_t in range(len(l_tags)):
                        if l_tags[i_t] in text_for_tg:
                            if l_tags[i_t] == '</li>' or l_tags[i_t] == '</td>':
                                tag = '\n'
                            else:
                                tag = ''
            ...
            t = variables_json['f']
            preview_home_teams = preview_home_teams.replace("+", " ").replace("'", "").replace(",", "").replace("[",
                                                                                                                "").replace(
                "]", "").split()
            preview_away_teams = preview_away_teams.replace("+", " ").replace("'", "").replace(",", "").replace("[",
                                                                                                                "").replace(
                "]", "").split()
            ...
            with open(
                    f'/opt/footballBot/parameters/football/users/text/{list_id[global_for]}_preview_round.json',
                    encoding='utf-8'
            ) as file:
                user_text = json.load(file)
            ...
            """ Preview """
            list_text = [
                f"Write a headline using the example.\nExample: Match day {rounds_for_text} in {league_name} will start on {first_date_round}",
                f"Write a text about this data in the example. \nExample: The first match of Match day {rounds_for_text} in {league_name} will be in {city_first_match} on {first_date_round} (UTC). {home_first_match} will host {away_first_match}. ",
                f"Write a text about this data in the example. \nExample: According to bookmakers the most important match is {main_match_for_bk_team_home} vs {main_match_for_bk_team_away}. {main_match_for_bk_team_home} win — {top_win_home}, draw — {top_draw}, {main_match_for_bk_team_away} win {top_lose_home}. ",
                f"Write a text about this data in the example. \nExample: Most goals in the league — {team_max_goals_league} scored the most goals in league ({max_goals_league} goals). {team_min_conceded_league} has the best defense ({min_conceded_top_saves} goals conceded). The most clean sheets — {team_max_clean_sheet_league}, in {max_cleen_sheet_league} games. {team_max_conceded_league} — the worst defense ({max_conceded_saves_league} goals conceded), the worst attack — {team_min_goals_attack_league} ({min_goals_attack_league} scored). {team_max_without_scored_league} didn’t scored in the most of games ({max_without_scored_league}). {team_max_conceded_goals_league} has the most game with goals conceded ({max_conceded_goals_league}).",
                f"Write a text about this data in the example. \nExample: The topscorers of the {league_name} now: {name_top_goals_league_1} – {goals_top_league_1} ({team_top_goals_league1}), {name_top_goals_league_2} - {goals_top_league_2}  ({team_top_goals_league2}), {name_top_goals_league_3} – {goals_top_league_3} ({team_top_goals_league3})."
            ]
            main_text = ""
            main_text_list = []
            index_openai = 0
            ...
            for i in ['"', "'"]:
                title_openAI = title_openAI.replace(i, "") if i in title_openAI else title_openAI
            first_paragraph, second_paragraph, threeth_paragraph, fourth_paragraph = main_text_list

            l1 = ['{first_paragraph}', '{second_paragraph}', '{threeth_paragraph}', '{fourth_paragraph}',
                  '{best_team_of_the_season}', '{count_matchs}', '{rounds_for_text}', '{league_id}',
                  '{city_first_match}', '{venue_first_match}',
            ...
            Делаем публикацию
            _____"""
            result_publish = publish(list_id[global_for], league_name, featured_image, title_openAI, main_text+text_all, types, rounds,
                                     client_language, league_id)
            ...
            if 'telegram' in data_j['auto-posting'] and result_publish != False:
                ...
                l_tags = ['<i>HW - home win, AW - away win, D - draw.</i></tbody></table>',
                          '<table  valign="top"><tbody><tr style= "background-color: #aedbea;"><th rowspan="2">Teams</th><th rowspan="2">Date</th><th colspan="3" rowspan="1">odds</th></tr><tr style= "background-color: #aedbea;"><th>HW</th><th>D</th><th>AW</th></tr>',
                          '<b>', '<p>', '<li>', '</b>', '</p>', '</li>', '<ul>', '</ul>', '<tr>', '<td>', '</td>',
                          '</tr>']
